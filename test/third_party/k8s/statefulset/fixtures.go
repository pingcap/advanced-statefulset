...
func ResumeNextPod(c clientset.Interface, ss *appsv1.StatefulSet) {
	...
	for _, pod := range podList.Items {
		if pod.Status.Phase != v1.PodRunning {
			k8s.Failf("Found pod in phase %q, cannot resume", pod.Status.Phase)
		}
		if podutils.IsPodReady(&pod) || !hasPauseProbe(&pod) {
			continue
		}
		if resumedPod != "" {
			k8s.Failf("Found multiple paused stateful pods: %v and %v", pod.Name, resumedPod)
		}
		_, err := k8s.RunHostCmdWithRetries(pod.Namespace, pod.Name, "dd if=/dev/zero of=/data/statefulset-continue bs=1 count=1 conv=fsync", StatefulSetPoll, StatefulPodTimeout)
		k8s.ExpectNoError(err)
		k8s.Logf("Resumed pod %v", pod.Name)
		resumedPod = pod.Name
	}
}
