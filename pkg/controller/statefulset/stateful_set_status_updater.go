...
func NewRealStatefulSetStatusUpdater(
	...
	setLister statefulsetlisters.StatefulSetLister) StatefulSetStatusUpdaterInterface {
	return &realStatefulSetStatusUpdater{client, setLister}
}

type realStatefulSetStatusUpdater struct {
	client    clientset.Interface
	setLister statefulsetlisters.StatefulSetLister
}
...
func (ssu *realStatefulSetStatusUpdater) UpdateStatefulSetStatus(
	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		set.Status = *status
		_, updateErr := ssu.client.PingcapV1alpha1().StatefulSets(set.Namespace).UpdateStatus(set)
		if updateErr == nil {
			return nil
		}
