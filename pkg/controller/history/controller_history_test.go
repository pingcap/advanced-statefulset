...
import (
	...
	fake "github.com/cofyc/advanced-statefulset/pkg/client/clientset/versioned/fake"
	statefulsetinformers "github.com/cofyc/advanced-statefulset/pkg/client/informers/externalversions"
...
func TestRealHistory_ListControllerRevisions(t *testing.T) {
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		informerFactory := statefulsetinformers.NewSharedInformerFactory(client, controller.NoResyncPeriodFunc())

		stop := make(chan struct{})
		defer close(stop)
		informerFactory.Start(stop)
		informer := informerFactory.Pingcap().V1alpha1().ControllerRevisions()
...
func TestRealHistory_CreateControllerRevision(t *testing.T) {
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		for _, item := range test.existing {
			_, err := client.PingcapV1alpha1().ControllerRevisions(item.parent.GetNamespace()).Create(item.revision)
			if err != nil {
				t.Fatal(err)
			}
		}
